...
The runtime enforces an ALLOWLIST of seven tools: read, grep, glob, list, webfetch, websearch, AND bash (read-only commands only — filtered by a command allowlist + operator block). Any other tool — including any MCP server's tools (mcp__*, playwright_*, zai-mcp-server_*, anything the user registered globally) and any browser/automation/vision tool — terminates the session on first call. Vision / image-analysis comes from the LLM provider's native multimodal capability built into the message stream, never from an external tool. If a tool isn't in the seven-name allowlist, do not call it.
