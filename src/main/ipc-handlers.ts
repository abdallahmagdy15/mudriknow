...
contextBlock += `\n  ${formatElementType(el.type)}`;
        ...
        if (attachScreenshotNext && (currentContext.imagePath || areaImagePath)) {
          if (screenshotMode === "chromium-auto" && screenInfo) {
            ...
            const cellW = geo.cellWPhys;
            const cellH = geo.cellHPhys;
            contextBlock += `\n\n[A full-screen screenshot is attached with a faint numbered coordinate grid overlay. Each grid cell is approximately ${cellW}×${cellH} physical pixels. Top-left cell is (0,0). The image itself is ${geo.imgW}×${geo.imgH} pixels — a downscale of the ${si.physicalWidth}×${si.physicalHeight} physical screen; report ALL coordinates in physical pixels (physical = image position × ${(si.physicalWidth / geo.imgW).toFixed(2)}). When estimating a position you MUST derive BOTH axes from the grid: x = column × ${cellW} + offset-within-cell, y = row × ${cellH} + offset-within-cell. Read the printed 0-indexed column labels along the top edge and row labels along the left edge — do NOT count bands by eye and do NOT guess from screen-region intuition. Self-check: if either coordinate is below one cell (${cellW} for x, ${cellH} for y) while the target is not inside cell 0, you skipped the grid — redo. This is a Chromium/Electron app — the UIA accessibility tree may NOT expose web content. THE SCREENSHOT IS THE PRIMARY SOURCE. Screen: ${si.physicalWidth}×${si.physicalHeight} pixels (DPI scale ${si.scaleFactor}×). Coordinate frame: left≈0, right≈${si.physicalWidth}, top≈0, bottom≈${si.physicalHeight}. The UIA layout tree above may only show shell chrome — trust the screenshot for actual page content.]`;
          } else if (screenInfo) {
            const si = screenInfo;
            const geo = computeGridGeometry(si.physicalWidth, si.physicalHeight);
            const cellW = geo.cellWPhys;
            const cellH = geo.cellHPhys;
            contextBlock += `\n\n[A screenshot is attached with a faint numbered coordinate grid overlay. Each grid cell is approximately ${cellW}×${cellH} physical pixels. Top-left cell is (0,0). The image itself is ${geo.imgW}×${geo.imgH} pixels — a downscale of the ${si.physicalWidth}×${si.physicalHeight} physical screen; report ALL coordinates in physical pixels (physical = image position × ${(si.physicalWidth / geo.imgW).toFixed(2)}). When estimating a position you MUST derive BOTH axes from the grid: x = column × ${cellW} + offset-within-cell, y = row × ${cellH} + offset-within-cell. Read the printed 0-indexed edge labels — do NOT count bands by eye and do NOT guess from screen-region intuition. Self-check: a coordinate below one cell size while the target is not inside cell 0 means you skipped the grid — redo. Screen: ${si.physicalWidth}×${si.physicalHeight} pixels @${si.scaleFactor}× DPI scale.]`;
          } else {
