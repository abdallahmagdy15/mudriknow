...
1. **Sandboxed agent.** MudrikNow spawns OpenCode with `--agent readonly` and provisions `.opencode/agent/readonly.md` into the working directory on startup. The agent declares `bash`, `edit`, `write`, `webfetch`, `websearch`, `task`, `todowrite`, and `skill` as denied.
